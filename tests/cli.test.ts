...
describe('parseCli', () => {
  ...
  it('rejects openrouter-status with a non-numeric timeout', () => {
    expect(parseCli(['openrouter-status', '--timeout', 'nope'])).toEqual({ mode: 'error', error: 'openrouter-status accepts only --timeout MS' });
  });
